...
func main() {
	...
	router.POST("/douyin/favorite/action", func(c *gin.Context) {
		...
		if actionType == "1" {         //为视频点赞
			err = client.ZAdd(user_id, redis.Z{Score: 1, Member: videoInfo}).Err() //用户的点赞，要记录对于用户来说，ta点赞了多少个视频，有哪些视频被点赞了
			if err != nil {
				panic(err)
				return
			}
			...
			err = client.ZRem(user_id, redis.Z{Score: 1, Member: videoInfo}).Err() //用户的点赞，要记录对于用户来说，ta点赞了多少个视频，有哪些视频被点赞了
			if err != nil {
				panic(err)
				return
			}
	...
	router.GET("/douyin/favorite/list/", func(c *gin.Context) {
		user_id := "123456" //待接收
		videos, err := client.ZRange(user_id, 0, -1).Result()
		if err != nil {
			panic(err)
		}
